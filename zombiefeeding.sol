...
contract ZombieFeeding is ZombieFactory {
  ...
  modifier ownerOf(uint _zombieId){ // creates modifier to check the owner
  ...
  function _isReady(Zombie storage _zombie) internal view returns (bool) { // check if cooldown has ended 
    ...
    function feedAndMultiply(uint _zombieId, uint _targetDna, string memory _species) internal ownerOf(_zombieId) { // function for feeding the zombie on cryptokittie
